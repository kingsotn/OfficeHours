...
void check(priority_queue<int> &pq, int& time_begin, const int& time, int* total_time, double& wait, queue<int> &arrive) {
  if (time == (pq.top() + time_begin)) { //checks if student can be popped off
    pq.pop();
    arrive.pop();
    cout << "pop! " << time << endl;
    if (pq.size() >= 1) {
  ...
  if (pq.top() + time > *total_time) {  // checks if teacher will need to stay past 60+ min
...
for (int i = 0; i < 100; i++){ // loop 100 times
  ...
  *total_time = 60;
  ...
  for (int time = 0; time <= *total_time; time++) { //actual loop
    int x;
    if (arrive() && time <= 60) { //will a student arrive at this minute?
      if (pq.empty())
        time_begin = time;
      t_arrive.push(time);
    ...
    } if (!pq.empty()) { // if q is not empty then check if student can be popped
        check(pq,time_begin,time,total_time, wait, t_arrive);
    } else if ( pq.empty() && time == 60) { // if 60 min have passed but q empty;
      break;
    }
